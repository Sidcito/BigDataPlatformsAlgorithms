...
public class PageRank {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < args.length; i += 2) {
                ...
                // NOTE: please do not switch in order to maintain the Java 1.6 compatibility!
                
                if (key.equals(KEY_DAMPING) || key.equals(KEY_DAMPING_ALIAS)) {
                    // make sure to have a damping factor in the interval [0:0.85]
                    PageRank.DAMPING = Math.max(Math.min(Double.parseDouble(value), 0.85), 0.0);
                } else if (key.equals(KEY_COUNT) || key.equals(KEY_COUNT_ALIAS)) {
                    // make sure to have at least one iteration for the PageRank algorithm
    ...
    /**
     * Printing the main an only help text 
     */
    public static void printUsageText(String err) {
        ...
        System.out.println("    " + KEY_OUTPUT + "   (" + KEY_OUTPUT_ALIAS + ")    <output>      The directory of the output [REQUIRED]");
